...
var CartModel = AbstractModel.extend({
    ...
    addBonusProduct: function (bonusDiscountLineItem, product, selectedOptions, quantity) {
        ...
        var UpdateProductOptionSelections = require('app_gmo_core/cartridge/scripts/cart/UpdateProductOptionSelections');
    ...
    validateForCheckout: function () {
        var ValidateCartForCheckout = require('app_gmo_core/cartridge/scripts/cart/ValidateCartForCheckout');
    ...
    createOrder: function () {
        ...
        var order;
        try {
        	if (dw.system.Site.getCurrent().getCustomPreferenceValue('gmo_postpay_enablesmscert') &&
				session.getCustom().smsauth) {
        		order = Transaction.wrap(function () {
	                return OrderMgr.createOrder(basket, session.getCustom().smsauth.buyer.shopTransactionId);
	            });
        	} else {
	            order = Transaction.wrap(function () {
