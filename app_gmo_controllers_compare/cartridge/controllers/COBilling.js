...
function returnToForm(cart, params) {
    ...
    var gmoCreditController = int_gmo.getController('GMO_PAYMENT');
	let resourceKeys        = ['single', 'installments', 'bonus', 'installments_bonus', 'revolving'];
	let resourceValues      = ['1', '2', '3', '4', '5'];
	let keyName;
    var gmoPayInfo          = gmoCreditController.GetCreditCardPaymentsInfo();
	var gmoPayResource      = {};
	...
	for(let key in resourceKeys) {
    	keyName = resourceKeys[key];
		gmoPayResource[keyName] = {
				name    : Resource.msg('payinfo.gmo_' + keyName + '_payment_name', 'forms', null),
				keyword : keyName,
				value   : resourceValues[key]
		};
	}

    if (params) {
    	params.gmoPayInfo = gmoPayInfo;
    	params.gmoPayResource = gmoPayResource;
        ...
        app.getView({
            Basket: cart.object,
            ContinueURL: URLUtils.https('COBilling-Billing'),
        	gmoPayInfo: gmoPayInfo,
        	gmoPayResource: gmoPayResource
        }).render('checkout/billing/billing');
    }
}
...
function initCreditCardList(cart) {
    ...
    if (customer.authenticated) {
        var gmoCreditController = int_gmo.getController('GMO_PAYMENT');
        var isEnableGmo         = gmoCreditController.IsEnableGMO();

        if(isEnableGmo.isEnableCreditCard){
            // getCardList
            pdictobj  = gmoCreditController.GetCardList(cart.object);

            if (pdictobj.applicableCreditCards != null) {
                return {
                	gmoCreditCard: pdictobj.gmoCreditCard,
                	ApplicablePaymentMethods: applicablePaymentMethods,
                    ApplicableCreditCards: pdictobj.applicableCreditCards
                };
            }else{
                return {
                	ApplicablePaymentMethods: applicablePaymentMethods,
...
function resetPaymentForms() {
    ...
    var status = Transaction.wrap(function () {
        if (app.getForm('billing').object.paymentMethods.selectedPaymentMethodID.value.equals('PayPal')) {
            ...
            cart.removePaymentInstruments(cart.getPaymentInstruments('RAKUTENID'));
            cart.removePaymentInstruments(cart.getPaymentInstruments('GMOPOSTPAY'));
            ...
            cart.removePaymentInstruments(cart.getPaymentInstruments('RAKUTENID'));
            cart.removePaymentInstruments(cart.getPaymentInstruments('GMOPOSTPAY'));
        } else if(app.getForm('billing').object.paymentMethods.selectedPaymentMethodID.value.equals('RAKUTENID')) {
            app.getForm('billing').object.paymentMethods.creditCard.clearFormElement();
            app.getForm('billing').object.paymentMethods.bml.clearFormElement();
            ...
            cart.removePaymentInstruments(cart.getPaymentInstruments('PayPal'));
            cart.removePaymentInstruments(cart.getPaymentInstruments('GMOPOSTPAY'));
        } else if(app.getForm('billing').object.paymentMethods.selectedPaymentMethodID.value.equals('GMOPOSTPAY')) {
            app.getForm('billing').object.paymentMethods.creditCard.clearFormElement();
            app.getForm('billing').object.paymentMethods.bml.clearFormElement();
...
function handlePaymentSelection(cart) {
    ...
    if (!result) {
        if (app.getForm('billing').object.paymentMethods.selectedPaymentMethodID.value == 'GMOPOSTPAY') {
            let gmoCreditController = int_gmo.getController('GMO_PAYMENT');
            let isEnableGmo         = gmoCreditController.IsEnableGMO();
            if (isEnableGmo.isEnableSMS) {
                let gmoPaymentType = 2; //別送
                if (isEnableGmo.isEnableEnclose && (cart.shipments.getLength() == 1)) { //同梱サービス契約有 && 配送先が単一
                    let  ship = cart.shipments[0].shippingAddress.fullName +
                        cart.shipments[0].shippingAddress.stateCode +
                        cart.shipments[0].shippingAddress.city +
                        cart.shipments[0].shippingAddress.address1;
                    if ( cart.shipments[0].shippingAddress.address2 ) {
                        ship += cart.shipments[0].shippingAddress.address2;
                    }
                    let  billing = cart.billingAddress.fullName +
                        cart.billingAddress.stateCode +
                        cart.billingAddress.city +
                        cart.billingAddress.address1;
                    if ( cart.billingAddress.address2 ) {
                        billing += cart.billingAddress.address2;
                    }

                    if (ship.eqal(billing)) { //配送先と請求先が同じ
                        gmoPaymentType = 3; //同梱
                    }
                }
                // Assign
                Transaction.wrap(function () {
                	cart.getCustom().gmoPaymentType = gmoPaymentType;
                });
            }
        }
...
function billing() {
    ...
    logger.debug('COBilling.billing()');
    ...
    app.getForm('billing').handleAction({
        ...
        save: function () {
            ...
            if (!resetPaymentForms() || (!validateBilling() && !isEnableGmo.isEnableCommon) || !handleBillingAddress(cart) || // Performs validation steps, based upon the entered billing address
            // and address options.
            handlePaymentSelection(cart).error) {// Performs payment method specific checks, such as credit card verification.

                if (!validateBilling() && !isEnableGmo.isEnableCreditCard) {
                    logger.debug("validateBilling=false,isEnableGmo=false");
                }

                returnToForm(cart);
            } else {

                logger.debug("validateBilling=false,isEnableGmo=false   ***NOT***");
